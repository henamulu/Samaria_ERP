    ...
    <div class="min-h-screen bg-slate-900">
        ...
        <div class="ml-64 p-8">
            <div class="flex justify-between items-center mb-8">
                <div>
                    <h2 class="text-3xl font-bold text-white">Purchase Orders</h2>
                    <p class="text-slate-400">Track all purchase order records</p>
                </div>
                <a href="/purchase-orders/create" class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg flex items-center gap-2">
            ...
            <div class="bg-slate-800 rounded-xl p-4 mb-6">
                <form @submit.prevent="applyFilters" class="flex gap-4">
                    <input v-model="filterForm.search" type="text" placeholder="Search by PO No..." 
                    ...
                    <a href="/purchase-orders" class="bg-slate-600 hover:bg-slate-500 text-white px-4 py-2 rounded-lg">Clear</a>
                </form>
            </div>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-slate-700">
                        <tr v-for="po in purchaseOrders.data" :key="po.id" class="hover:bg-slate-700/50">
                            <td class="px-4 py-4 text-white font-medium">{{ po.po_no }}</td>
                            <td class="px-4 py-4 text-slate-300">{{ po.registered_date }}</td>
                            <td class="px-4 py-4 text-slate-300">{{ po.item }}</td>
                            <td class="px-4 py-4 text-slate-300">{{ po.quantity }} {{ po.unit }}</td>
                            <td class="px-4 py-4 text-slate-300">{{ formatCurrency(po.unit_price) }}</td>
                            <td class="px-4 py-4 text-emerald-400 font-bold">{{ formatCurrency(po.total) }}</td>
                            <td class="px-4 py-4"><span :class="statusClass(po.status)" class="px-2 py-1 rounded text-xs">{{ po.status }}</span></td>
                            <td class="px-4 py-4">
                                <div class="flex gap-2">
                                    <a :href="'/purchase-orders/' + po.id + '/edit'" class="text-blue-400 hover:text-blue-300">Edit</a>
                                    <button @click="confirmDelete(po)" class="text-red-400 hover:text-red-300">Delete</button>
                                </div>
                            </td>
                        </tr>
                    </tbody>
            ...
            <div class="flex justify-between items-center mt-6 text-slate-400">
                <span>Showing {{ purchaseOrders.from }}-{{ purchaseOrders.to }} of {{ purchaseOrders.total }}</span>
                <div class="flex gap-2">
                    <a v-if="purchaseOrders.prev_page_url" :href="purchaseOrders.prev_page_url" class="px-4 py-2 bg-slate-700 rounded hover:bg-slate-600">Previous</a>
                    <a v-if="purchaseOrders.next_page_url" :href="purchaseOrders.next_page_url" class="px-4 py-2 bg-slate-700 rounded hover:bg-slate-600">Next</a>
                </div>
            </div>
        </div>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-slate-800 rounded-xl p-6 max-w-md w-full mx-4">
                ...
                <p class="text-slate-300 mb-6">Are you sure you want to delete purchase order "{{ deleteTarget?.po_no }}"?</p>
...
const applyFilters = () => {
    router.get('/purchase-orders', filterForm.value, { preserveState: true });
};
...
const deleteItem = () => {
    router.delete('/purchase-orders/' + deleteTarget.value.id, {
        onSuccess: () => {
