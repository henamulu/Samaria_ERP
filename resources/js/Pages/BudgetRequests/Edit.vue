    ...
    <div class="min-h-screen bg-slate-900">
        ...
        <div class="ml-64 p-8">
            <div class="mb-6 flex justify-between items-center">
                <div>
                    <h2 class="text-3xl font-bold text-white mb-2">Edit Budget Request</h2>
                    <p class="text-slate-400">Update budget request details</p>
                </div>
                ...
                <form @submit.prevent="submitForm">
                    ...
                    <div class="mt-6 flex gap-3">
                        <button type="submit" :disabled="loading" class="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-3 rounded-lg disabled:opacity-50">
                            {{ loading ? 'Updating...' : 'Update Budget Request' }}
                        </button>
...
onMounted(() => {
    if (props.budgetRequest) {
        form.value = {
            item_id: props.budgetRequest.item_id || '',
            unit: props.budgetRequest.unit || '',
            unit_price: parseFloat(props.budgetRequest.unit_price) || 0,
            quantity: parseFloat(props.budgetRequest.quantity) || 0,
            profit: parseFloat(props.budgetRequest.profit) || 0,
            overhead: parseFloat(props.budgetRequest.overhead) || 0,
        };
    }
});
...
const submitForm = () => {
    loading.value = true;
    router.put(`/budget-requests/${props.budgetRequest.id}`, form.value, {
        preserveState: false,
        onFinish: () => {
            loading.value = false;
        }
