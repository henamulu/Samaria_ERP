    ...
    <div class="min-h-screen bg-slate-900">
        ...
        <div class="ml-64 p-8">
            <div class="mb-6 flex justify-between items-center">
                <div>
                    <h2 class="text-3xl font-bold text-white mb-2">Purchase Balance Report</h2>
                    <p class="text-slate-400">Outstanding purchase order balances</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="bg-slate-800 rounded-xl p-6">
                    <p class="text-slate-400 text-sm mb-1">Total Purchase Orders</p>
                ...
                <div class="bg-slate-800 rounded-xl p-6">
                    ...
                    <p class="text-3xl font-bold text-red-400">{{ formatCurrency(totalBalance) }}</p>
                </div>
            </div>
            ...
            <div class="bg-slate-800 rounded-xl overflow-hidden">
                <div class="p-4 border-b border-slate-700 flex justify-between items-center">
                    <h3 class="text-lg font-semibold text-white">Purchase Orders with Outstanding Balance</h3>
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-slate-700">
                            <tr v-for="po in purchaseOrders.data" :key="po.id" class="hover:bg-slate-700/50">
                                ...
                                <td class="px-4 py-3 text-slate-300">{{ formatCurrency(po.total) }}</td>
                                <td class="px-4 py-3 text-red-400 font-medium">{{ formatCurrency(po.c_balance) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
const applyFilters = () => {
    router.get('/reports/purchase-balance', filters.value, {
        preserveState: true,
        preserveScroll: true
    });
};
...
const exportCSV = () => {
    const headers = ['PO No', 'Date', 'Supplier', 'Item', 'Quantity', 'Total', 'Balance'];
    ...
    a.href = url;
    a.download = `purchase_balance_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
