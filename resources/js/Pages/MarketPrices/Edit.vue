    ...
    <div class="min-h-screen bg-slate-900">
        ...
        <div class="ml-64 p-8">
            <div class="mb-6 flex justify-between items-center">
                <div>
                    <h2 class="text-3xl font-bold text-white mb-2">Edit Market Price</h2>
                    <p class="text-slate-400">Update sales price information</p>
                </div>
...
const customerAgreements = ref(props.agreements || []);
...
onMounted(() => {
    if (props.price) {
        form.value = {
            customer: props.price.customer || '',
            price_type: props.price.price_type || '',
            agg_no: props.price.agg_no || '',
            item_id: props.price.item_id || '',
            unit: props.price.unit || '',
            unit_price: parseFloat(props.price.unit_price) || 0,
            tax_p: parseFloat(props.price.tax_p) || 15,
            transport: props.price.transport || 'No',
            transport_unit: props.price.transport_unit || '',
            t_unit_price: parseFloat(props.price.t_unit_price) || 0,
            t_tax_p: props.price.t_tax_p || ''
        };
...
const submitForm = () => {
    loading.value = true;
    router.put(`/market-prices/${props.price.id}`, form.value, {
        preserveState: false,
        onFinish: () => {
            loading.value = false;
        }
