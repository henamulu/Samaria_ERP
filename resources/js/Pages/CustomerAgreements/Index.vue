    ...
    <div class="min-h-screen bg-slate-900">
        ...
        <div class="ml-64 p-8">
            <div class="flex justify-between items-center mb-8">
                <div>
                    <h2 class="text-3xl font-bold text-white">Customer Agreements</h2>
                    <p class="text-slate-400">Manage customer price agreements</p>
                </div>
                <a href="/customer-agreements/create" class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg flex items-center gap-2">
            ...
            <div class="bg-slate-800 rounded-xl p-4 mb-6">
                <form @submit.prevent="applyFilters" class="flex gap-4">
                    <input v-model="filterForm.search" type="text" placeholder="Search by Agreement No., Customer..." 
                    ...
                    <a href="/customer-agreements" class="bg-slate-600 hover:bg-slate-500 text-white px-4 py-2 rounded-lg">Clear</a>
                </form>
            </div>
                ...
                <table class="w-full">
                    <thead class="bg-slate-700">
                        <tr>
                            ...
                            <th class="px-4 py-4 text-left text-xs font-medium text-slate-300 uppercase">Transport</th>
                            <th class="px-4 py-4 text-left text-xs font-medium text-slate-300 uppercase">Tax %</th>
                    ...
                    <tbody class="divide-y divide-slate-700">
                        <tr v-for="(agg, index) in agreements.data" :key="agg.id" class="hover:bg-slate-700/50">
                            ...
                            <td class="px-4 py-4 text-slate-300">{{ agg.customer?.company_name || agg.customer?.firstname }}</td>
                            ...
                            <td class="px-4 py-4 text-slate-300">{{ agg.transport_unit }} @ {{ formatCurrency(agg.t_unit_price) }}</td>
                            <td class="px-4 py-4 text-slate-300">{{ agg.tax_p }}</td>
                            <td class="px-4 py-4"><span :class="statusClass(agg.status)" class="px-2 py-1 rounded text-xs">{{ agg.status }}</span></td>
                            <td class="px-4 py-4 text-slate-400 text-sm">{{ agg.registered_date }}<br/>{{ agg.registered_by }}</td>
                            <td class="px-4 py-4">
                                <div class="flex gap-2">
                                    <a :href="'/customer-agreements/' + agg.id + '/edit'" class="text-blue-400 hover:text-blue-300">Edit</a>
        ...
        <div v-if="showVoidModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-slate-800 rounded-xl p-6 max-w-md w-full mx-4">
                ...
                <p class="text-slate-300 mb-6">Are you sure you want to void agreement "{{ voidTarget?.agg_no }}"? This action cannot be undone.</p>
...
const applyFilters = () => {
    router.get('/customer-agreements', filterForm.value, { preserveState: true });
};
...
const voidAgreement = () => {
    router.put('/customer-agreements/' + voidTarget.value.id + '/void', {}, {
        onSuccess: () => {
